...
$(document).ready(function() {
 ...
 $(document).on('click', '#filter', function(e){
    ...
    var exportUrl = "{{ URL::to('iuran/report/pdf') }}?start_date="+startDate+"&end_date="+endDate;
    ...
    $.ajax({
            ...
            success:function(response){
                ...
                if (response.status == 200) {
                    $("#modal").modal('show');
                    $(".modal-title").text("Edit Iuran");
                    $('#id_form').val("1");
                    $('#iuran_id').val(data.id);
                    $('#tanggal').val(data.tanggal);
                    $('#nominal').val(data.nominal);
                    $('#status').val(data.status);
                    $('#jenis_iuran').val(data.jenis_iuran);
                    $('select[name=status] option').filter(':selected').val(data.status);
                    $('#anggota_id').val(data.anggota_id);
                    $('#anggota_id option').filter(':selected').val(data.anggota_id);
                }
            }, 
        });
});
...
var url         = "{{ route ('api.iuran') }}";
var token       = "{{Auth::user()->api_token}}";
var can_create  = "{{ Auth::user()->can('iuran-create') }}";
var can_edit    = "{{ Auth::user()->can('iuran-edit') }}";
var can_delete  = "{{ Auth::user()->can('iuran-delete') }}";
...
$('#modal_form').submit(function(e){
    ...
    var iuran_id     = $('#iuran_id').val();
    var url_store    = "{{ URL::to('iuran/store') }}";
    var url_update   = "{{ URL::to('iuran/update') }}/"+iuran_id;
    ...
    var anggota_id =  $('select[name=anggota_id] option').filter(':selected').val();
    var jenis_iuran  = $('input[name="jenis_iuran"]').val();
    ...
    formData.append('jenis_iuran', jenis_iuran);
        $.ajax({
            headers:{
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
...
<div class="card-header bg-transparent header-elements-inline">
        ...
        <a href="{{ URL::to('iuran/report/pdf') }}" target="_blank" id="periode"  class="btn bg-success ml-sm-2 mb-sm-0 mr-2 filter"> <i class="icon-file-pdf mr-2"></i> Download PDF</a>
        </div>
    ...
    <table class="table datatable-button-init-custom">
        <thead>
            <tr>
                ...
                <th>Anggota</th>
                <th>Jenis Iuran</th>
                @if(Gate::check('iuran-edit') || Gate::check('iuran-delete'))
