...
@section('init')
    <script>
        ...
        $(document).on('click', '#filter', function(e) {
            ...
            var exportUrl = "{{ URL::to('pengeluaran/report/pdf') }}?start_date=" + startDate + "&end_date=" +
                endDate;
            $('#periode').attr('href', exportUrl);
            console.log(exportUrl);

            e.preventDefault();
            $('.datatable-button-init-custom').DataTable().ajax.url(url + "?api_token=" + token + "&start_date=" +
                ...
                },
                {
                    ...
                    "render": function(data, type, row) {
                        ...
                        if (data.bukti == null || data.bukti == '') {
                            imgBukti =
                                '<a href="#" class="btn btn-icon bg-transparent btn-sm border-slate-300 text-slate rounded-round border-dashed"><i class="icon-image2"></i></a>';
                        } else {
                            imgBukti = '<a href="' + path + data.bukti +
                                '" target="_blank" data-popup="lightbox"><img src="' + path + data.bukti +
        ...
        $('#modal_form').submit(function(e) {
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    ...
                    new PNotify({
                        title: 'Gagal !',
                        text: 'Gagal mengubah data pengeluaran',
            ...
            <div class="card">
                <div class="card-header bg-transparent header-elements-inline">
                    <div class="card-title">
                        <a href="#"
                            class="btn bg-primary ml-sm-2 mb-sm-0 sidebar-right-main-toggle sidebar-component-toggle"> <i
                                class="icon-filter4 mr-2"></i> Filter </a>
                        <a href="{{ URL::to('pengeluaran/report/pdf') }}" target="_blank" id="periode"
                            class="btn bg-success ml-sm-2 mb-sm-0 mr-2 filter"> <i class="icon-file-pdf mr-2"></i> Download
                            PDF</a>
                    </div>
